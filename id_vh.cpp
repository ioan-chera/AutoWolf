...
{
	VWL_MeasureString(string,width,height,(fontstruct *)grsegs[gfxvmap[STARTFONT][SPEAR.Flag()]+fontnumber]);
}
...
{
	VL_MemToScreen (((byte *)grsegs[gfxvmap[STARTTILE8M][SPEAR.Flag()]])+tile*64,8,8,x,y);
}
...
{
	int	picnum = chunknum - gfxvmap[STARTPICS][SPEAR.Flag()];
...
{
	VL_LatchToScreen (latchpics[2+picnum-gfxvmap[LATCHPICS_LUMP_START][SPEAR.Flag()]], x*8, y);
}
...
{
	VL_LatchToScreenScaledCoord (latchpics[2+picnum-gfxvmap[LATCHPICS_LUMP_START][SPEAR.Flag()]], scx*8, scy);
}
...
{
    int i;
    for(i = 0; i < (2 + (signed int)gfxvmap[LATCHPICS_LUMP_END][SPEAR.Flag()] - (signed int)gfxvmap[LATCHPICS_LUMP_START][SPEAR.Flag()]); i++)
    {
...
//
    surf = SDL_CreateRGBSurface(SDL_HWSURFACE, 8*8,
        ((gfxvmap[NUMTILE8][SPEAR.Flag()] + 7) / 8) * 8, 8, 0, 0, 0, 0);
    if(surf == NULL)
    {
	...
	latchpics[0] = surf;
	CA_CacheGrChunk (gfxvmap[STARTTILE8][SPEAR.Flag()]);
	src = grsegs[gfxvmap[STARTTILE8][SPEAR.Flag()]];

	for (i=0;i<(signed int)gfxvmap[NUMTILE8][SPEAR.Flag()];i++)
	{
		VL_MemToLatch (src, 8, 8, surf, (i & 7) * 8, (i >> 3) * 8);
		src += 64;
	}
	UNCACHEGRCHUNK (gfxvmap[STARTTILE8][SPEAR.Flag()]);
...
// pics
//
	start = gfxvmap[LATCHPICS_LUMP_START][SPEAR.Flag()];
	end = gfxvmap[LATCHPICS_LUMP_END][SPEAR.Flag()];

	for (i=start;i<=end;i++)
	{
		width = pictable[i-gfxvmap[STARTPICS][SPEAR.Flag()]].width;
		height = pictable[i-gfxvmap[STARTPICS][SPEAR.Flag()]].height;
